...
.DESCRIPTION
    ...
    The service runs `node server.js` from C:\apps\embrace-ai.

.NOTES
    Run after: setup-server.ps1, git clone, npm install
#>
...
Write-Host "`n=== Installing EmbraceAI as Windows Service ===" -ForegroundColor Cyan
...
& $NssmPath set $ServiceName AppRotateBytes 5242880  # 5 MB rotation
...
if ($svc.Status -eq "Running") {
    ...
    Write-Host "  App available at: http://localhost:3000`n" -ForegroundColor Green
} else {
    Write-Host "`n  Service status: $($svc.Status)" -ForegroundColor Red
    Write-Host "  Check logs at: $LogDir`n" -ForegroundColor Yellow
}

Write-Host @"
Service management commands:
  nssm start EmbraceAI       — Start the service
  nssm stop EmbraceAI        — Stop the service
  nssm restart EmbraceAI     — Restart the service
  nssm status EmbraceAI      — Check status
  nssm edit EmbraceAI        — Edit service config (GUI)
